...
- [x] T009 Register services in `src/NatsManager.Web/Program.cs`: `builder.Services.Configure<MonitoringOptions>(...)`, `builder.Services.AddHttpClient()`, `builder.Services.AddSignalR()`, register `IMonitoringAdapter` → `NatsMonitoringHttpAdapter` (singleton), register `IMonitoringMetricsStore` → `MonitoringMetricsStore` (singleton)
...
- [ ] T026 [P] [US2] Frontend component tests in `src/NatsManager.Frontend/src/features/monitoring/components/JetStreamMetricsCard.test.tsx`: renders stream/consumer/message/bytes counts from `latestSnapshot.jetStream`; renders unavailable state message when no usable JetStream snapshot exists; renders area chart with message/byte trends from `snapshots`

### Implementation for User Story 2

- [x] T027 [US2] Implement `JetStreamMetricsCard` component in `src/NatsManager.Frontend/src/features/monitoring/components/JetStreamMetricsCard.tsx`: Mantine `Card` with `SimpleGrid` of 4 stat cells (stream count, consumer count, total messages, total bytes with formatted large numbers); below the stats, a Recharts `<AreaChart>` plotting `totalMessages` and `totalBytes` over time using the `snapshots` array; renders a Mantine `Alert` with info icon when `jetStream === null` (JetStream not enabled)
- [x] T028 [US2] Add `JetStreamMetricsCard` to `MonitoringPage` in `src/NatsManager.Frontend/src/features/monitoring/MonitoringPage.tsx`: render `JetStreamMetricsCard` below `ServerMetricsChart`, passing `snapshots`; wrap in a `Stack` with a "JetStream" section title
